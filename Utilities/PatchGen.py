'''
PatchGen contains functions for image patch generation used in deep learning model training
...
def LoadHdf5Data(hdf5Filename):    
  ...
  with h5py.File(hdf5Filename, 'r') as f:
    ...
    for k in f['patches'].attrs.keys():
      #hdf5Data['numberOfScales']=f['patches'].attrs['numberOfScales']
...
# %% Symbolic links to split samples into test and train

def SymLinkFilesToDir(fileList,targetDir):
  for sourceFile in fileList:
    targetFile=os.path.join(targetDir,os.path.split(sourceFile)[-1])
    os.symlink(sourceFile,targetFile)    

def SplitTestTrain(trainFrac,fullFileList,trainDir,testDir):
  
  assert trainFrac>=0 and trainFrac<=1
  nFile=len(fullFileList)
  assert nFile>0
  nTrain = int(np.ceil(trainFrac*nFile))
  shuffledFileList=np.array(fullFileList.copy())
  np.random.shuffle(shuffledFileList)
  trainList=shuffledFileList[np.arange(nTrain)]
  testList=shuffledFileList[np.arange(nTrain,nFile)]
  
  if(not os.path.isdir(trainDir)):
    os.makedirs(trainDir)
  SymLinkFilesToDir(trainList,trainDir)   
  
  if(not os.path.isdir(testDir)):
    os.makedirs(testDir)
  SymLinkFilesToDir(testList,testDir)   
